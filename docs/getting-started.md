...
**Not MGFA certification:** DriftGuard provides contract observability evidence — not identity, HITL, or policy evaluation. See [semantic drift boundary](./guides/semantic-drift-boundary.md) · [integration boundaries](./reference/hosted-api.md#integration-boundaries).
...
**Full loop tutorial:** After the harness runs locally, continue with the [contract observability loop](https://developers.driftguard.org/docs/guides/contract-observability-loop) — baseline → CI gate → optional hosted watch and alerts.
...
> **Clone fallback** (contributors or until `@drift-guard` is on npm):
>
> ```bash
...
- [ ] `npx @drift-guard/driftguard@latest version` (or clone + `npm ci && npm run build`)
